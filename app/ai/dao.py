...
from app.models import (
    Dish, Order, OrderStatus, Review, SentimentLabel, UserDishInteraction,
)
...
def get_review_for_order_dish(order, dish_id):
    return (
        Review.query
        .filter(Review.order_id == order.id, Review.dish_id == dish_id)
        .first()
    )
...
def get_reviewed_dish_ids_for_orders(order_ids):
    if not order_ids:
        return {}
    rows = (
        db.session.query(Review.order_id, Review.dish_id)
...
def get_dish_rating_stats(dish_id):
    avg, count = (
        db.session
        .query(db.func.avg(Review.rating), db.func.count(Review.id))
...
def add_review(user_id, order, dish_id, rating, comment, sentiment):
    """Lưu đánh giá với sentiment là (label, score) hoặc None."""
    ...
    db.session.add(UserDishInteraction(
        user_id=user_id, dish_id=dish_id, interaction_type='REVIEW',
        hour_of_day=datetime.now().hour,
    ))
    try:
        db.session.commit()
...
def dish_exists(dish_id):
    return db.session.get(Dish, dish_id) is not None
