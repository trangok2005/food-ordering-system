...
def my_orders():
    ...
    reviewed = ai_dao.get_reviewed_dish_ids_for_orders([o.id for o in orders])

    return render_template('my_orders.html', orders=orders, reviewed=reviewed)
...
def cart_stats_api():
    ...
    stats = dao.get_cart_stats(current_user.id)
...
def add_to_cart():
    ...
    try:
        ...
        return_path = urlparse(referrer).path if referrer else None
...
def confirm_switch():
    ...
    if not (return_url and return_url.startswith('/') and not return_url.startswith('//')):
        return_url = None

    try:
        dao.clear_all_carts(current_user.id)
        dao.add_to_cart(current_user.id, dish_id, quantity)
        flash('Đã xóa giỏ cũ và thêm món mới')
    except ValueError as e:
        flash(str(e), 'error')

    return redirect(return_url or url_for('cart.cart_view'))
